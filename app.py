...
from skops.io import load as skops_load  # 專門讀取 scikit-learn 物件

st.set_page_config(page_title="Cox Risk Predictor", page_icon="⏳", layout="centered")

# ==== Cox loss (載入模型需要；推論其實不會用到) ====
...
@st.cache_resource
def load_artifacts():
    preprocessor = skops_load("preprocessor.skops", trusted=True)
...
categorical_vars = [
    "sex", "grade", "margin", "LVI", "PNI",
    "pT", "pN", "cysticneoplasm", "neoadjuvant", "opmethod"
]
...
cont_inputs["ca199"] = st.number_input("ca199", min_value=0.0, max_value=100000.0, value=100.0, step=1.0)
cont_inputs["size"]  = st.number_input("size", min_value=0.0, max_value=500.0, value=30.0, step=1.0)

# 類別變數（你可以依據資料集實際類別修改選項）
cat_inputs = {}
...
if st.button("預測風險分數"):
    ...
    if hasattr(X, "toarray"):  # 若是 sparse matrix，轉 dense
        X = X.toarray()
